...
gemtCookieFile = os.path.join(os.path.dirname(os.path.realpath(__file__)), "cookies")
gemtSERVER = ''
session = ''
...
class gemtLogin(sublime_plugin.ApplicationCommand):
	def __init__(self):
		self.email = ""
	...
	def run(self):
		...
		sublime.active_window().show_input_panel("Email:", info['Email'], self.getEmail, None, None)

	def getEmail(self, email):
		email = email.strip()
		if len(email)>0:
			sublime.active_window().show_input_panel("Password:", "", self.getPassword, None, None)
			self.email = email
		else:
	...
	def getPassword(self, password):
		...
		if len(password)>0:
			...
			url = urllib.parse.urljoin(gemtSERVER, 'auth/api/login')
			...
			try:
				with urllib.request.urlopen(req, None, gemtTIMEOUT) as response:
					...
					if rsp['code'] != 200:
						...
						print(ck['GEMPY_session'].value)
						session = ck['GEMPY_session'].value
						session_expiration_time = datetime.datetime.now() + datetime.timedelta(hours=2)
						with open(gemtCookieFile, 'wb') as f:
							pickle.dump({'GEMPY_session': session, 'session_expiration': session_expiration_time}, f)
						return
			except urllib.error.HTTPError as err:
				sublime.message_dialog("{0}".format(err))
			except urllib.error.URLError as err:
